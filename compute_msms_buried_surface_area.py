...
for pdbfile in allPDB_files:
  xyzrn_filenames.append(`xyzrn_count`)
  output_pdb_as_xyzrn(pdbfile, xyzrn_filename_template.format(`xyzrn_count`))
  xyzrn_count += 1

# Now run MSMS for every xyzrn file
for xyzrn_filename in xyzrn_filenames:
  FNULL = open(os.devnull, 'w')
  full_filename = xyzrn_filename_template.format(xyzrn_filename)
...
complex_area_filename = area_filename_template.format(xyzrn_count-1)
...
# each unbound file.
for file_ix in range(1, xyzrn_count-1):
  chain_ses_file = open(area_filename_template.format(file_ix))
  ...
  vert_filename = env.tmpDirectory+"{}.xyzrn.vert".format(file_ix)
  ...
  face_filename = env.tmpDirectory+"{}.xyzrn.face".format(file_ix)
  ...
  out_file_vertices = open(env.outputDirectory+`file_ix`+".vert", 'w')
  ...
  out_file_face = open(env.outputDirectory+`file_ix`+".face", 'w')
